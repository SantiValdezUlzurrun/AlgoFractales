...
#        self.orientacion -= math.radians(180)
#    def girar_izquierda(self, cantidad):
#        self.orientacion -= math.radians(cantidad)
#    def girar_derecha(self, cantidad):
#        self.orientacion += math.radians(cantidad)
#    def avanzar(self,cantidad = [10,10]):
#        self.posicion[0] -= cantidad[0] * math.cos(self.orientacion)
#        self.posicion[1] -= cantidad[1] * math.sin(self.orientacion)
#    def circulo_svg(self):
#       return '<circle cx="15" cy="10" r="8" fill="white" />'
#    def pasar_linea_svg(self):
#        a = self.posicion_inicial[:] 
#        c = self.posicion[:]
#        return f' <line x1="{a[0]}" y1="{a[1]}" x2="{c[0]}" y2="{c[1]}" stroke-width="{self.ancho}" stroke="{self.color}" />'
#    def __repr__(self):
